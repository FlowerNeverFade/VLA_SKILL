...
LEROBOT_RECORDS_FILENAME = "lerobot_records.json"
_LEROBOT_DATASET_CACHE: dict[tuple[str, str, str, bool, bool], LeRobotDataset] = {}
_LEROBOT_VALIDATED_DATASET_KEYS: set[tuple[str, str, str, bool, bool]] = set()
_LEROBOT_BACKEND_CACHE: dict[tuple[str, str, str], tuple[str, str | None]] = {}
...
def _load_lerobot_task_name(skill_spec: SkillSpec) -> str:
    ...
    tasks_parquet_path = source.dataset_dir / "meta" / "tasks.parquet"
    if tasks_parquet_path.is_file():
        try:
            import pandas as pd
        except ImportError as exc:  # pragma: no cover - pandas is available in the training env
            raise SkillDataError("Reading tasks.parquet requires pandas.") from exc
        tasks_df = pd.read_parquet(tasks_parquet_path)
        if "task_index" not in tasks_df.columns:
            raise SkillDataError(f"{tasks_parquet_path} must contain a `task_index` column.")
        matches = tasks_df[tasks_df["task_index"].astype(int) == int(source.task_index)]
        if matches.empty:
            raise SkillDataError(f"Task index {source.task_index} not found in {tasks_parquet_path}.")
        return str(matches.index[0])

    raise SkillDataError(
        f"Could not find task metadata for skill `{skill_spec.skill_id}`; expected "
        f"{tasks_path} or {tasks_parquet_path}."
    )


def _load_precomputed_lerobot_episode_records(
    skill_spec: SkillSpec,
) -> tuple[dict[str, LeRobotEpisodeRecord], str | None, str | None] | None:
    if skill_spec.skill_dir is None:
        return None
    records_path = skill_spec.skill_dir / LEROBOT_RECORDS_FILENAME
    if not records_path.is_file():
        return None
    payload = load_json(records_path)
    task_name = str(payload.get("task_name") or _load_lerobot_task_name(skill_spec))
    resolved_backend = payload.get("resolved_video_backend")
    fallback_reason = payload.get("fallback_reason")
    records: dict[str, LeRobotEpisodeRecord] = {}
    for episode_id, item in (payload.get("records") or {}).items():
        frame_indices = tuple(int(value) for value in item["frame_indices"])
        records[str(episode_id)] = LeRobotEpisodeRecord(
            episode_id=str(episode_id),
            source_episode_index=int(item["source_episode_index"]),
            frame_indices=frame_indices,
            num_frames=int(item.get("num_frames", len(frame_indices))),
            fps=float(item["fps"]),
            task=task_name,
        )
    if not records:
        raise SkillDataError(f"{records_path} contains zero precomputed episode records.")
    return records, str(resolved_backend) if resolved_backend is not None else None, fallback_reason
...
def clear_lerobot_dataset_cache() -> None:
    _LEROBOT_DATASET_CACHE.clear()
    _LEROBOT_VALIDATED_DATASET_KEYS.clear()
    _LEROBOT_BACKEND_CACHE.clear()


def _lerobot_source_key(skill_spec: SkillSpec, video_backend: str) -> tuple[str, str, str, bool, bool]:
    source = skill_spec.source
    assert source.dataset_dir is not None
    return (
        source.repo_id,
        str(source.dataset_dir),
        video_backend,
        False,
        True,
    )
...
def _lerobot_backend_cache_key(skill_spec: SkillSpec) -> tuple[str, str, str]:
    source = skill_spec.source
    assert source.dataset_dir is not None
    return source.repo_id, str(source.dataset_dir), source.video_backend


def _get_lerobot_dataset(skill_spec: SkillSpec, video_backend: str) -> LeRobotDataset:
    key = _lerobot_source_key(skill_spec, video_backend)
    dataset = _LEROBOT_DATASET_CACHE.get(key)
    if dataset is None:
        dataset = LeRobotDataset(**_lerobot_dataset_kwargs(skill_spec, video_backend))
        _LEROBOT_DATASET_CACHE[key] = dataset
    return dataset


def _validate_lerobot_dataset_once(skill_spec: SkillSpec, video_backend: str) -> LeRobotDataset:
    key = _lerobot_source_key(skill_spec, video_backend)
    dataset = _get_lerobot_dataset(skill_spec, video_backend)
    if key not in _LEROBOT_VALIDATED_DATASET_KEYS:
        _ = dataset[0]
        _LEROBOT_VALIDATED_DATASET_KEYS.add(key)
    return dataset


def resolve_lerobot_video_backend(skill_spec: SkillSpec) -> tuple[str, str | None]:
    backend_cache_key = _lerobot_backend_cache_key(skill_spec)
    cached = _LEROBOT_BACKEND_CACHE.get(backend_cache_key)
    if cached is not None:
        return cached

    requested_backend = skill_spec.source.video_backend
    if requested_backend != "auto":
        _validate_lerobot_dataset_once(skill_spec, requested_backend)
        resolved = (requested_backend, None)
        _LEROBOT_BACKEND_CACHE[backend_cache_key] = resolved
        return resolved

    try:
        _validate_lerobot_dataset_once(skill_spec, "torchcodec")
        resolved = ("torchcodec", None)
        _LEROBOT_BACKEND_CACHE[backend_cache_key] = resolved
        return resolved
    except Exception as exc:  # noqa: BLE001
        fallback_reason = f"{type(exc).__name__}: {exc}"

    _validate_lerobot_dataset_once(skill_spec, "pyav")
    resolved = ("pyav", fallback_reason)
    _LEROBOT_BACKEND_CACHE[backend_cache_key] = resolved
    return resolved
...
def build_lerobot_dataset(skill_spec: SkillSpec) -> tuple[LeRobotDataset, str, str | None]:
    ...
    dataset = _get_lerobot_dataset(skill_spec, resolved_backend)
...
def collect_lerobot_episode_records(
    ...
    precomputed = _load_precomputed_lerobot_episode_records(skill_spec)
    if precomputed is not None:
        records, resolved_backend, fallback_reason = precomputed
        return records, resolved_backend or skill_spec.source.video_backend, fallback_reason
